...
function mockFetch(body, ok = true, status = 200, statusText = "OK") {
...
describe("useProduct", () => {
    it("returns the product on successful fetch", async () => {
        const { result } = renderHook(() => useProduct("https://api.test.com", "1"));
    ...
    it("calls the correct URL: apiUrl + /api/products/ + productId", async () => {
        ...
        const { result } = renderHook(() => useProduct("https://api.test.com", "42"));
        await waitFor(() => expect(result.current.loading).toBe(false));
        expect(spy).toHaveBeenCalledWith("https://api.test.com/api/products/42", expect.anything());
    });
});
