...
function mockFetch(product: Partial<Product> = MOCK_PRODUCT, ok = true, status = 200) {
...
describe("ProductCard", () => {
  it("muestra skeleton mientras carga", () => {
    ...
    render(<ProductCard apiUrl={API_URL} productId="1" />);
  ...
  it("muestra precio con descuento cuando product.discount > 0", async () => {
    ...
    render(<ProductCard apiUrl={API_URL} productId="1" />);
    await waitFor(() =>
      document.querySelector(".qc-card__price--discount")
    );
  ...
  it("llama onAddToCart con el producto al hacer click en el botón", async () => {
    ...
    render(
      <ProductCard apiUrl={API_URL} productId="1" onAddToCart={onAddToCart} />
    );
  ...
  it("aplica className extra al elemento raíz", async () => {
    render(
      <ProductCard apiUrl={API_URL} productId="1" className="mi-clase" />
    );
    await waitFor(() =>
      document.querySelector(".qc-card.mi-clase")
    );
  ...
  it("aplica theme como CSS variables inline", async () => {
    ...
    render(
      <ProductCard apiUrl={API_URL} productId="1" theme={theme} />
    );
    await waitFor(() => screen.getByText("Zapatillas Air Max"));
    const card = document.querySelector(".qc-card") as HTMLElement;
